<!-- resources/views/outputs/index.blade.php -->
@extends('layouts.app')
    ...
    <div class="w-full space-y-8">
        <div
            ...
            <div>
                <h1 class="text-4xl font-black tracking-tighter text-gray-800 dark:text-gray-50 uppercase">Histórico de
                    Saídas</h1>
                <p class="text-gray-500 font-medium mt-1 uppercase text-[10px] tracking-widest dark:text-gray-400">Registro
                    de movimentações e vendas comissionadas.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('saidas.create') }}" target="_blank"
                    class="flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl transition-all shadow-lg shadow-emerald-100 dark:shadow-none cursor-pointer text-[10px] uppercase tracking-widest">
                    <i data-lucide="shopping-cart" class="w-4 h-4"></i> Abrir Frente de Caixa
                </a>
            </div>
        </div>

        <form action="{{ route('saidas') }}" method="GET"
            ...
            <div class="md:col-span-2 relative">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="PRODUTO, SKU OU VENDEDOR..."
        ...
        <div
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="px-8 py-5">Produto / Vendedor</th>
                            <th class="px-8 py-5 text-center">Qtd</th>
                            <th class="px-8 py-5 text-right">Total</th>
                            <th class="px-8 py-5 text-center">Pagamento</th>
                            ...
                            <tr class="group hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-all">
                                ...
                                <td class="px-8 py-5">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm font-black text-gray-800 dark:text-gray-50 tracking-tight uppercase">{{ $output->product->name }}</span>
                                        <span class="text-[10px] text-emerald-500 font-mono font-bold uppercase">Resp:
                                            {{ $output->employee->name ?? 'SISTEMA' }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-center">
                                    <span
                                        class="px-3 py-1 bg-gray-100 dark:bg-gray-900 rounded-lg font-black text-xs">{{ $output->quantity }}</span>
                                </td>
                                ...
                                <td class="px-8 py-5 text-center">
                                    <span
                                        class="text-[9px] font-black uppercase px-2 py-1 border border-gray-200 dark:border-gray-800 rounded-md text-gray-500 dark:text-gray-400">
                                        {{ $output->payment_method }}
                                        {{ $output->installments > 1 ? "({$output->installments}x)" : '' }}
                                    </span>
                                </td>
                                <td class="px-8 py-5 text-center">
                                    <a href="{{ route('saidas.recibo', $output->id) }}" target="_blank"
                                        class="p-2 text-gray-400 hover:text-emerald-500 transition-colors inline-block cursor-pointer">
                            ...
                            <tr>
                                <td colspan="6" class="px-8 py-12 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Nenhuma venda
                                            encontrada para os filtros.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div
                ...
                <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Exibindo
                    {{ $outputs->count() }} de {{ $outputs->total() }} registros</span>
                {{ $outputs->links() }}
            </div>
        </div>
    </div>
    ...
    @if(session('last_sale_id'))
        <script>
            window.onload = function () {
                const url = "{{ route('saidas.recibo', session('last_sale_id')) }}";
